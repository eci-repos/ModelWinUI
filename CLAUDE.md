...
The code lives in **seven layered, independently-reusable class libraries** under `src/` (backlogs 030 + 031 + 034). Each builds standalone (`dotnet build src/Model.X/Model.X.csproj -c Debug`); the app references the collection. Dependencies point only downward — no cycles:
...
├── Model.Skia/            net10.0, SkiaSharp             — generic Skia drawing + ERD renderer + factory contracts (→ Model.Graph, Model.Data, Model.Geometry)
├── Model.Graphics.WinUI/  net10.0-windows, WindowsAppSDK — the XAML Gl* drawing stack + ERD Table renderer + XAML factory contracts (→ Model.Data, Model.Graph, Model.Geometry, Model.Diagnostics)
