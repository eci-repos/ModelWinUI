...
The code lives in **six layered, independently-reusable class libraries** under `src/` (backlogs 030 + 031). Each builds standalone (`dotnet build src/Model.X/Model.X.csproj -c Debug`); the app references the collection. Dependencies point only downward — no cycles:
...
└── Model.WinUI.Console/   the app (→ all six)
```

**Namespace identity (backlogs 032 + 031):** each library owns a single clean namespace and **no namespace is declared by two assemblies** — `ModelConsole.Geometry` (Model.Geometry), `ModelConsole.Graph` + `ModelConsole.Editing` (Model.Graph), `ModelConsole.Diagnostics` (Model.Diagnostics, home of `ILogService`/`LogService`), `ModelConsole.Skia.Services` (Model.Skia's factory contracts), `ModelConsole.Graphics.GLibrary` + `ModelConsole.Graphics.Services` (Model.Graphics.WinUI — the XAML stack **and** its `ITableFactory`/`IConnectorFactory`/`IRectangleFactory`), while the app keeps `ModelConsole.Services` (its `IModelDataProvider`). Each library also ships a README (`src/Model.X/README.md`) and NuGet packaging metadata (`Model.Console.<Layer>`, `Version 0.1.0`, README embedded via `PackageReadmeFile`) — `dotnet pack src/Model.X/Model.X.csproj` is clean.
