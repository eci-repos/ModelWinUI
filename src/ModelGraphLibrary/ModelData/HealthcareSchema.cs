namespace ModelConsole.ModelData
{
   ...
   /// <summary>
   /// The 020 gate fixture: a hand-authored, deliberately messy grouped model
   /// in the healthcare clinic domain, written the way a third party would —
   /// the Entity / Elements / "Depends On" vocabulary, inconsistent type
   /// strings, an ambiguous name that exercises R7 precedence (declared beats
   /// inferred), and two dependencies to the same entity that need explicit
   /// roles. The shipped <c>Samples/Healthcare.json</c> is this exact
   /// document; the gate tests interpret it through the built-in grouped
   /// profile with no code updates to the renderers or explorer.
   /// </summary>
