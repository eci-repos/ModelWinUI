...
1. **Schema-driven model interpretation (backlog `020`–`022`)** — `019` (interpreter core) is **done** (2026-08-18, archived; sprint promoted). Next is **`020` (proof sample model — the gate)**: author a third-party-style model in a new domain with the grouped `$.entities` shape and the `Entity`/`Elements`/`Depends On` vocabulary (deliberately messy: inconsistent type strings, an ambiguous name exercising R7, a two-dependencies-to-one-entity case needing an explicit role, a small enumeration, metadata + provenance), ship it as a sample, and load it through `019` with **no code updates**. Then `021` (type + enum wiring) → `022` (cardinality/metadata/provenance readout). v2 (generalization, uniqueness, referential-integrity, stereotypes) is designed-for, not built, until the `020` gate passes. `docs/sprints/CURRENT.md` is a placeholder seeded with `020`.
