...
1. **Node enrichment series (backlog `026` → `025`)** — the follow-up series created 2026-08-18 from user design feedback. `023` (containerized model structure; flat forms stay readable) and `024` (descriptions on entities/elements, 116/116 tests) are **done** (2026-08-18). Next: `026` adds per-node provenance on tables/columns (currently model-level only), then `025` ships a JSON Schema per representation + load-time validation. `docs/sprints/CURRENT.md` holds the current item (026).
